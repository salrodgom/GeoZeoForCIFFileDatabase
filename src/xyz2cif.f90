program xyz2cif
 ...
 read (5,*) &
 rv(1,1),rv(2,1),rv(3,1),rv(1,2),rv(2,2),rv(3,2),rv(1,3),rv(2,3),rv(3,3),line
 spacegroup = "p1"
 call inverse(rv,vr,3)
 call uncell(rv,cell_0)
 read_coor_pdb: do i=1,n_atoms
  if (ierr/=0) exit read_coor_pdb          ! formatos de lectura para pdb 
  id(i) = i
  atomc = ' '; mol = 'MOL'
  read(5,*) label(id(i)), xinbox(1,id(i)), xinbox(2,id(i)), xinbox(3,id(i)), k
  !forall ( j = 1:3 )
...
subroutine escritura_cif(xcryst,n_atoms,label,cell_0,rv)
 ...
 u=1000
 open(u,file="p1_fromPDFFile.cif")
 ...
 atoms_: do i=1,n_atoms
  write(u,'(2(a4,2x),1x,4(f10.8,1x))')label(i),label(i),xcryst(1,i),xcryst(2,i),xcryst(3,i),0.0
 end do atoms_
 close(u)
 return
end subroutine escritura_cif
!
subroutine cell_from_matrix(rv,vr)
 implicit none
 real, intent(in)  :: rv(3,3)
 real, intent(out) :: vr(3,3)
 call inverse(rv,vr,3)
end subroutine
!
